...
const Dashboard = () => {
  const { backendUrl, token, navigate, userProfile, updateUserProfile, uploadProfileImage, logout, products, currency, coupons, notifications, unreadNotifications, markNotificationsRead } = useContext(ShopContext);
  ...
  const formatDate = (ts: number) => {
    return new Date(Number(ts)).toLocaleDateString() + ' · ' + new Date(Number(ts)).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
  };

  const notificationMeta: Record<string, { icon: React.ReactNode; tint: string }> = {
                  ...
                  return (
                    <div key={n._id} className={`flex items-start gap-3 rounded-2xl border p-3.5 text-sm transition-colors ${n.read ? 'border-gold/10 bg-white/50 opacity-70' : 'border-gold/30 bg-gold/10'}`}>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-ink-soft/60 mt-1.5">{formatDate(n.createdAt)}</p>
                      </div>
              ...
              <div key={coupon._id} className={`relative overflow-hidden rounded-2xl border border-gold/20 bg-white/70 card-lux ${i > 1 ? 'hidden sm:block' : ''}`}>
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between gap-2 mt-3">
                    <p className="font-display text-xl gold-text font-bold">{coupon.discountType === 'percent' ? `${coupon.discountValue}% off` : `Rs. ${coupon.discountValue} off`}</p>
                    <button
                  ...
                  <div className="flex items-center justify-between mt-3 pt-3 border-t border-gold/10 text-[11px] text-ink-soft">
                    <span>{coupon.minPurchase > 0 ? `Min. spend Rs. ${coupon.minPurchase}` : 'No minimum spend'}</span>
