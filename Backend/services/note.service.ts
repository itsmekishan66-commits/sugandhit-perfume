...
const defaultNotes: Array<{ name: string; layer: string; icon: string; color: string; description: string }> = [
...
const defaultBases: Array<{ name: string; code: string; description: string; extraPrice: string }> = [
...
export const getPaletteStock = async () => {
  ...
  return {
    ...
    bases: bases.map((b) => ({ ...b, extraPrice: parseFloat(b.extraPrice as string) })),
  };
};